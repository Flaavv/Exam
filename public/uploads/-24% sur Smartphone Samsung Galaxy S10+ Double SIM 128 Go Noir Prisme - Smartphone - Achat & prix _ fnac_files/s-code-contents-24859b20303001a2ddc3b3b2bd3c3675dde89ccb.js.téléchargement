// 18/12/2019 : Ligne 125 - Suppresion de l'écriture de l'evar 66 dans le cookie s_ev66. Cookie trop volumineux. 

// Use by react pages (datalayer is dtm_datalayer variable)
function mboxLoadSCPlugin(){}
 
if (window.dtm_datalayer && window.dtm_datalayer.account && window.dtm_datalayer.account.indexOf('fnac') !== -1){
 s_account = window.dtm_datalayer.account;
}
...
if(document.location.host.indexOf('fnac.ch')>-1)
	s.currencyCode = "CHF";
...
s.linkInternalFilters = "javascript:,.fnac.com,ogone.com,fd-recette.net,limonetikqualif.com,oney.com"+window.location.host;

if(document.location.host.indexOf('fnac.be')>-1)
  s.linkInternalFilters = "javascript:,ogone.com,fnac.be";

if(document.location.host.indexOf('fnac.es')>-1)
  s.linkInternalFilters = "javascript:,ogone.com,fnac.es";

if(document.location.host.indexOf('fnac.pt')>-1)
  s.linkInternalFilters = "javascript:,ogone.com,fnac.pt";

if(document.location.host.indexOf('fnac.ch')>-1)
  s.linkInternalFilters = "javascript:,ogone.com,fnac.ch";
...
s.cookieLifetime="33696000"; // sets cookie expiration to D+390
...
if(urlTmp.indexOf('#',0)>0){
	...
	s.pageURL=urlTmp;
/*End Rebuilt standard query string in the url*/

function s_doPlugins(s) {
	/* Add calls to plugins here */
  
	/* Kameleeon tracking*/
  if (s.pageName && s.pageName == "Erreur Javascript"){
  	s.eVar103=window.dtm_datalayer.error_message.message;
		s.eVar104=window.dtm_datalayer.error_message.stack;
  }
	
  /* get bot referrer */		
s.eVar96=s.getQueryParam('fnacbot');
  
   /* s.tracksent=true;*/
	// Kameleoon tracking
	try{
		window.Kameleoon && Kameleoon.API && Kameleoon.API.Tracking.processOmniture(s);
	}catch(e){
		console.warn(e)
	}
 
	/* Campaign tracking; do only on the first run of s_doPlugins */
	if (s.p_fo() == 1)
	{
    ...
    s.eVar46 = s.getQueryParam('tmail');
    s.eVar67 = s.getQueryParam('gclid');
		
		if((document.referrer.indexOf("nl.fnac.be")>0)&&(window.location.hostname=="www.fr.fnac.be")&&!s.campaign)
		{
		s.campaign=GetCookie('campaign')
		}
		if((document.referrer.indexOf("fr.fnac.be")>0)&&(window.location.hostname=="www.nl.fnac.be")&&!s.campaign)
		{
		s.campaign=GetCookie('campaign')
		}
	}
	/*Stacking tracking codes - 8 TrackingCodes 30 Days Expire on Purchase*/
	if(s.campaign) {
		...
		s.eVar14 = s.crossVisitParticipation(stackcampaign, 's_ev14', '30', '8', '>', 'purchase');
	}

	// Set stacking cookie expiration to D+390
	var myDate=new Date();
	myDate.setDate(myDate.getDate()+390);
	s.c_w("s_ev14",s.c_r("s_ev14"),myDate); 
	
	/* Internal campaign tracking*/
  if(!s.eVar1){
  	var regxp_evar22 = /^.[:][a-zA-Z0-9]*/;
		s.eVar1= evar1 = s.getQueryParam('int');
		if (evar1.match(regxp_evar22)){
      evar1 = evar1.split('|');
			s.eVar22 = evar1[0] + '|' + evar1[1];
		}
  }
  
 	// Get cookie s_t to eVar66
	// 18/12/2019 : Les 3 lignes ci-dessous ont été commentés à cause du problème Safari suite à la MEP du 17/10. Le cookie s_ev66 devient trop volumeux et empeche la récupération/écriture des cookies dans les navigateurs.
	//if(!s.eVar66)
	//s.eVar66=GetCookie('TEST_R')
  //s.eVar66=s.getValOnce(s.eVar66,'s_ev66');

	/* Internal campaign stacking  tracking*/
s.eVar61 = s.crossVisitParticipation(s.eVar1, 's_ev61', '30', '2', '>', 'purchase',1);
	
	/* Set Product finding method for internal campaign */
	if(s.eVar1 && s.eVar1 != "Non internal campaign")
		s.eVar2='internal campaign';
		
	/* Internal Search Banner tracking*/
	if(s.getQueryParam('int_search_ban').toLowerCase()=="true")
	{
		s.events=s.apl(s.events,'event49',',',2);
		if (s.prop40.toLowerCase()!="fiche article")
			s.eVar2 = "search";
	}
	...
	// Check whether the referring domain is an internal domain
	...
	if (document.referrer)
	{
		...
		var params = location.hash.substr(1).split("&").reduce(function(result, pair) {
        var parts = pair.split("=");
        result[parts[0].toLowerCase()] = decodeURIComponent(parts.slice(1).join("="));
        return result
    }, {});
    if ("st" in params && "ct" in params && "t" in params) {
        s.prop10 = s.eVar10 = params.st;
        s.prop13 = params.ct;
        s.eVar2 = "search";
        s.eVar13 = "D=c13";
        s.eVar42 = "D=v2";
  ...
  /* Get internalflag=true on the strat reco on the basket page */
  if (s.eVar1 && s.eVar1.toLowerCase().indexOf("panierfinal")>-1)
    internalFlag = true;

	/* Get product position clicked in search list */
	if(s.prop40 && s.prop40.toLowerCase()=="fiche article"){
		s.eVar82 = s.getQueryParam('omnsearchpos');
	}
	
	/* Set the Product finding method for direct access to product page */
	if (s.prop40 && s.prop40.toLowerCase()=="fiche article" && !internalFlag)
		s.eVar2 = "direct access FA"
	
	/* Set the Merchandising Category variable */
	if (s.prop40 && s.eVar3 && s.prop40.toLowerCase()!="fiche article")
		s.eVar29 = "D=v3";

	
	/* Reset the Search variables for finding methods other than search */
	if (s.eVar2 && s.eVar2.toLowerCase()!="search")
	{
		s.eVar10 = s.eVar11 = s.eVar13 = s.eVar35 = "Non Search"
	...
	/* Reset the Merchandising Category variables for finding methods other than browse */
	if (s.eVar2 && s.eVar2.toLowerCase()!="browse")
	{
		s.eVar29 = "Non browse"
		s.linkTrackVars=s.apl(s.linkTrackVars,'eVar29',',',2);
	}
	
	/* Reset the Internal Campaigns variables for finding methods other than internal campaign */
	if (s.eVar2 && s.eVar2.toLowerCase()!="internal campaign")
	{
		s.eVar1 = s.eVar22 = "Non internal campaign"
	...
	//Ajout du scOpen en cas de scAdd (serialise à la visite dans le backend)
	if(s.events&&(s.events.indexOf('scAdd')>-1)){
		s.events = s.apl(s.events,'scOpen',',','2');
		s.linkTrackEvents = s.apl(s.linkTrackEvents,'scOpen',',','2');
	}
	
	// Ajout de l'evenement Purchase serialisé à la visite
	...
	//Set Merchandising instances for merchandising eVars
	if (s.eVar1||s.eVar2||s.eVar3||s.eVar6||s.eVar10||s.eVar11||s.eVar13||s.eVar19||s.eVar29||s.eVar35)
	{
		if (!s.products)
			s.products=";"

		s.linkTrackVars=s.apl(s.linkTrackVars,'products',',',2);
		s.linkTrackVars=s.apl(s.linkTrackVars,'event',',',2);
	...
	/* Definition des events de recrutements et renouvellements adherents */
	var PRIDsRecrutement = ["1124678","1324471","1328549","1328550","2022505","2022506","2022507","2022508","2022509","2022510","2022511","2022512","2022513","2816995","3077213"]
	var PRIDsRenouvellement = ["2022514","2022515","2022516","2022517","2022518","2022519","2022520","2022521","2022522","3076378","3077212"]

	for (i=0; i < PRIDsRecrutement.length; i++)
	{
		if (s.products && s.products.indexOf(PRIDsRecrutement[i])>=0 && s.events && s.events.indexOf("purchase")>=0)
			s.events = s.apl(s.events,'event47',',',2);
	}
	for (i=0; i < PRIDsRenouvellement.length; i++)
	{
		if (s.products && s.products.indexOf(PRIDsRenouvellement[i])>=0 && s.events && s.events.indexOf("purchase")>=0)
  ...
  2021:'3/28,10/31'}
	...
	s.prop30 = tpA[1]; // Set hour
	s.prop31 = tpA[2];  // Set day
	s.prop32 = tpA[3]; // Set Weekend / Weekday
	...
	//Template de page
	if(s.prop7){s.eVar34="D=c7";}
	
	
	// Premier mode d'acces produits
	if(s.eVar2){s.eVar42="D=v2";}
	...
	{
		s.eVar20="D=c20";
		s.linkTrackVars=s.apl(s.linkTrackVars,'eVar20',',',2);
		s.linkTrackVars=s.apl(s.linkTrackVars,'prop20',',',2);
	}
	/* ID de profiles pour conversion*/
	if(s.prop21)
	{
		s.eVar21="D=c21";
	...
	if (window.location.href)
		s.prop38 = window.location.href;
	
    //console.log(s.eVar83);
}

/* Variable product concaténation datalayer 
var products ="";

for (var i = 0; i < window.dtm_datalayer.products.length;i++){
products+=";"+
window.dtm_datalayer.products[i].prid+";"+
window.dtm_datalayer.products[i].quantity+";"+
window.dtm_datalayer.products[i].price+";";

if (window.dtm_datalayer.products[i].seller && window.dtm_datalayer.products[i].seller!==""){
	products+=";eVar7="+ window.dtm_datalayer.products[i].seller;
}

if (window.dtm_datalayer.products[i].productTypes && window.dtm_datalayer.products[i].productType!==""){
	products+="|eVar8="+ window.dtm_datalayer.products[i].productType;
}

if (window.dtm_datalayer.products[i].productStatus && window.dtm_datalayer.products[i].productStatus!==""){
	products+="|eVar9="+ window.dtm_datalayer.products[i].productStatus;
}

if (window.dtm_datalayer.products[i].expedition && window.dtm_datalayer.products[i].expedition!==""){
	products+="|eVar24="+ window.dtm_datalayer.products[i].expedition;
}

if (window.dtm_datalayer.products[i].codeStock && window.dtm_datalayer.products[i].codeStock!==""){
	products+="|eVar48="+ window.dtm_datalayer.products[i].codeStock;
}

if (window.dtm_datalayer.products[i].livraison && window.dtm_datalayer.products[i].livraison!==""){
	products+="|eVar23="+ window.dtm_datalayer.products[i].livraison;
}
if(i !== window.dtm_datalayer.products.length)
	products+=","
} */
...
s.trackingServer = s_trackingServer; //"metrics.fnac.com"
s.trackingServerSecure = s_trackingServerSecure; //"smetrics.fnac.com"
s.dc = 112;
